...
| `notch`                        | object           | none                                    | Cuts a rectangular bite out of the card's bottom-right corner so another card can tuck into the gap. `width`/`height` measure in from the right/bottom edges; both are required. Optional `shadow` replaces the card's `box-shadow`, which `clip-path` would otherwise clip away (give it a value with **no** spread radius). |
