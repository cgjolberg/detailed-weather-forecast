...
| `header_font_size`             | number \| string | theme default                           | Optional font size for the header condition/status text, e.g. `28`, `28px`, or `1.4rem`.                                                                                              |
| `temp_font_size`               | number \| string | theme default                           | Optional font size for the header temperature text, e.g. `56`, `56px`, or `3rem`.                                                                                                     |
| `header_icon_size`             | number \| string | `60px`                                  | Optional size for the header condition icon, e.g. `72`, `72px`, or `4rem`. Applies to the compact header (`show_background: false`).                                                   |
| `header_chip_font_size`        | number \| string | theme default                           | Optional font size for the header chips (sun/humidity/wind), e.g. `20`, `20px`. The chip icon scales with it.                                    |
| `header_line_height`           | number \| string | `normal`                                | Line height for the compact header's condition/temperature. Unitless values are passed through, so `1.05` tightens large type that would otherwise inherit ~1.6.  |
| `header_padding`               | number \| string | `16px`                                  | Padding around the compact header's condition/temperature row. A full CSS shorthand, so `12px 16px 6px` trims the row's height without costing the condition text horizontal room.                    |
| `card_min_height`              | number \| string | `200px`                                 | Overrides the card's minimum height (the `--card-height` floor). Useful when the card must fit an exact dashboard cell.                                          |
| `notch`                        | object           | none                                    | Cuts a rectangular bite out of the card's bottom-right corner so another card can tuck into the gap, and redraws the card's outline to run continuously around the resulting L. `width`/`height` measure in from the right/bottom edges; both are required. `corner_radius` rounds the cut's two convex corners and `inner_radius` its concave one (both default to the card's radius; see below). Optional `shadow` replaces the card's `box-shadow`, which `clip-path` would otherwise clip away (give it a value with **no** spread radius). |
